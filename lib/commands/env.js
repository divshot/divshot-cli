...
module.exports = function (cli) {
  ...
  command.handler(withApp(function (app, environment, done) {
    ...
    app.endpoint('env').one(environment + '/config').get(function(err, response) {
      ...
      if (response.status == 404) {
        return done(cli.errors.DIRECTORY_NOT_APP);
      }
  ...
  // // Configure environment (i.e. force ssl, etc.)
  ...
  command.task('remove <environment> KEY1 Key2 ...')
    ...
    .handler(withApp(function (app, environment, done) {
      ...
      app.env(environment).config({ env: vars }, function(err, response) {
        ...
        if (response.statusCode == 400) {
          return done(cli.errors.MUST_HAVE_DEPLOYED_APP);
        }
        ...
        cli.log('Values have been removed from the environment for ' + format.bold(environment), {success: true});
