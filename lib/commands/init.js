...
function initiateConfig (app) {
  app.logger.writeln();
  app.logger.info('Inititiate a new ' + app.format.blue('Divshot.io') + ' app.');
  app.logger.writeln();
  ...
  async.series({
    ...
    createApp: prompts.createApp
      ...
      app.logger.success(app.format.blue('Divshot.io') + ' app created');
  }));
}
...
var configComposer = {
  ...
  createApp: function (app) {
    return function (config, callback) {
      ...
      if (createApp) {
        app.api.apps.create(config.name, function (err, response) {
          console.log(arguments, config, config.name);
