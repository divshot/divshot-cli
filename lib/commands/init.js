...
module.exports = function (cli) {
  ...
  var prompts = {
    ...
    errorPage: function (callback) {
      promptly.prompt('error page: (error.html)', { default: false }, function (err, errorPage) {
        if (errorPage) return callback(err, errorPage);
        if (fs.existsSync(process.cwd() + '/error.html')) return callback(err, 'error.html');
        
        var errorPagePath = path.resolve(__dirname, '../templates/not_found.html');
        var errorPageContent = fs.readFileSync(errorPagePath)
        
        fs.writeFile(process.cwd() + '/error.html', errorPageContent.toString(), function (err) {
          callback(err, 'error.html');
        });        
      });
    },
