      ...
      tmp.dir({unsafeCleanup: true}, function(err, tmpDir) {
        async.map(filesToUpload, function(src, callback){
          ...
          uploader.on('error', function(err) {
            cli.log(err.stack);
            done("unable to deploy: " + err.message);
          });
          uploader.on('progress', function() {
            if(uploader.progressTotal == 0) return;
            if(uploader.progressAmount == 0) return;
            if(!progressBar) {
              var progressBar = new ProgressBar('Transferring: [' + format.green(':bar') + '] ' + format.bold(':percent'), {
                complete: '=',
                incomplete: ' ',
                width: 50,
            ...
            progressBar.tick(uploader.progressAmount);
