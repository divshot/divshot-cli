...
module.exports = function (cli) {
  var command = cli.command('rename');
  
  command.before('authenticate');
  command.description('change the name of the app');
  command.action(function (name, done) {
    var originalName = cli.config.name;
    ...
    promptly.confirm('Are you sure you want to rename this app? It will be permanant and change the app\'s url. (y/n)', function (err, rename) {
      ...
      cli.api.apps.id(cli.config.name).update({name: name}, function (err, response) {
