    ...
    streamDir(cwd)
      ...
      .on('end', function() {
        archive.finalize(function(error, bytes) {
          ...
          console.log("packed: " + bytes + " bytes.");
        });
      });

      var requestOptions = require('url').parse(buildData.loadpoint.zip_url)
      requestOptions.method = "PUT";
      requestOptions.headers = {
        Authorization: buildData.loadpoint.authorization
      }
      var request = require('http').request(requestOptions);
      archive.pipe(request);
      archive.on('data', function(data) {
        // error && console.error(error);
        console.log("ARCHIVE DATA", data.toString().length);
      });
      request
        .on('end', internals.finalizeBuild(environment, build, function (error) {
          process.exit(0);
        }));
  };
};
