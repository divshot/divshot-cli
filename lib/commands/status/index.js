...
module.exports = function (cli) {
  var command = cli.command('status');
  ...
  command.action(function (environment, done) {
    ...
    // Single
    if (typeof environment !== 'function') return getEnvironmentReleases(cli.config.name, environment); 
    
    // All
    done = environment; // Get statuses for all environments
    
    var envs = ['development', 'staging', 'production'];
    async.eachSeries(envs, function (env, next) {
      getIndividualReleaseInfo(cli.config.name, env, next);
